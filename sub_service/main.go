...
import (
	...
	transportgrpc "jianjustin/sub-grpc-service/transport/grpc"
	transporthttp "jianjustin/sub-grpc-service/transport/http"
	"net"
	http1 "net/http"
	"os"
	"os/signal"
...
func main() {
	...
	g.Go(func() error {
		return ServeHTTP(ctx, &endpoints, ":18081", log.With(logger, "transport", "HTTP"))
	})
...
func ServeGRPC(ctx context.Context, endpoints *transport.EndpointsSet, addr string, logger log.Logger) error {
	...
	pb2.RegisterSubServiceServer(grpcServer, server)
