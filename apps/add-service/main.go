...
func main() {
	...
	log.With(logger, "level", "info").Log("msg", fmt.Sprintf("grpc server start at 127.0.0.1:%v", *port))
...
func EtcdRegisterAddServiceMiddleware(logger log.Logger) service.AddServiceMiddleware {
	return func(next service.AddService) service.AddService {
		r := register2.GetEtcdRegister()
		if r == nil {
			log.With(logger, "level", "error").Log("msg", "get register client failed")
			return next
		}
		err := r.Register(etcdv3.Service{Key: register2.ServiceKey, Value: fmt.Sprintf("127.0.0.1:%d", *port)})
		if err != nil {
