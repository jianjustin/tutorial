...
import (
	...
	pb2 "go.guide/mul-grpc-service/pb"
...
var (
	...
	restful = flag.Int("restful", 8083, "the port to restful serve on")
)
...
func main() {
	...
	svc := service.NewMulService()
	svc = service.LoggingMulServiceMiddleware(logger)(svc)
	svc = EtcdRegisterMulServiceMiddleware(logger)(svc)
...
func ServeGRPC(ctx context.Context, endpoints *transport.EndpointsSet, addr string, logger log.Logger) error {
	...
	pb2.RegisterMulServiceServer(grpcServer, server)
...
func EtcdRegisterMulServiceMiddleware(logger log.Logger) service.MulServiceMiddleware {
	return func(next service.MulService) service.MulService {
